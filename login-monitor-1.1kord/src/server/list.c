...
// 将“pdata”插入到末尾位置
int linklist_append_last(linkList *plist, linknodeData *pdata);
...
static inline void free_linknodedata(linknode *pnode)
{
    if (NULL != pnode)
    {
        if (NULL != pnode->user_login_info)
        {
            linknodeData *pnodedata = (linknodeData *)pnode->user_login_info;
            if (NULL != pnodedata->login_user_name_monit)
            {
                free(pnodedata->login_user_name_monit);
                pnodedata->login_user_name_monit = NULL;
            }
            
            free(pnode->user_login_info);
            pnode->user_login_info = NULL;
        }
...
{
    linknodeData *data = (linknodeData *)pnode->user_login_info;
    printf("_id = %d, str = %s\n", data->login_user_name_monit, data->login_line);
}

static linknode *create_linknode(linknodeData *user_login_info)
{
    ...
    pnode->user_login_info=user_login_info;
    pnode->_next = NULL;
...
void *linklist_get_first(linkList *plist)
{
    ...
    return plist->_phead->user_login_info;
}
...
void *linklist_get_last(linkList *plist)
{
    ...
    return get_linknode(plist, plist->_count - 1)->user_login_info;
}
...
int linklist_insert_head(linkList *plist, linknodeData *pdata)
{
    if (NULL == plist)
    {
        return -1;
